...
# ── checkpoint config ──────────────────────────────────────────────────────
# label shown on button -> function that decides whether a given S3 key
...
CHECKPOINT_MATCHERS = {
    ...
    "Pre-Tournament": lambda key: key.endswith("_predictions.csv") and not any(tag in key for tag in ROUND_TAGS),
    "After Round 1":  lambda key: key.endswith("_r1_predictions.csv"),
    "After Round 2":  lambda key: key.endswith("_r2_predictions.csv"),
    "After Round 3":  lambda key: key.endswith("_r3_predictions.csv"),
}
DEFAULT_CHECKPOINT = "Pre-Tournament"
...
def get_latest_predictions(checkpoint_label: str):
    ...
    matcher = CHECKPOINT_MATCHERS[checkpoint_label]
    response = s3_client.list_objects_v2(Bucket=S3_BUCKET, Prefix="predictions/")
    files = sorted(
        [obj for obj in response.get("Contents", [])
         if matcher(obj["Key"])],
...
# ── checkpoint button row ───────────────────────────────────────────────────
cols = st.columns(len(CHECKPOINT_MATCHERS))
for col, label in zip(cols, CHECKPOINT_MATCHERS.keys()):
    is_active = st.session_state.checkpoint == label
    if col.button(
        label,
        use_container_width=True,
